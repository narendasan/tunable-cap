...
from generative_policy_proposals._atari_wrappers import (
    ClipRewardEnv,
    EpisodicLifeEnv,
    FireResetEnv,
    MaxAndSkipEnv,
...
class NoisyDuelingDistributionalNetwork(nn.Module):
    def __init__(self, env, n_atoms, v_min, v_max,):
        ...
        # Replace the feature extractor with the generated feature extractor from LLM
        ...
        self.value_head = nn.Sequential(NoisyLinear(10, 48), nn.ReLU(), NoisyLinear(48, n_atoms))

        self.advantage_head = nn.Sequential(
            NoisyLinear(10, 48), nn.ReLU(), NoisyLinear(48, n_atoms * self.n_actions)
        )

    def forward(self, x):
        #h = self.network(x / 255.0)
        # Need to normalize inputs (can they be zero centered?)
        h = x
...
class PrioritizedReplayBuffer:
    def __init__(self, capacity, obs_shape, device, n_step, gamma, alpha=0.6, beta=0.4, eps=1e-6):
        ...
        self.buffer_obs = (np.zeros((capacity,) + obs_shape, dtype=np.uint8), [None] * capacity)
        #self.buffer_next_obs = np.zeros((capacity,) + obs_shape, dtype=np.uint8)
        self.buffer_next_obs = (np.zeros((capacity,) + obs_shape, dtype=np.uint8), [None] * capacity)
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        idx = self.pos
        self.buffer_obs[0][idx] = obs[0]
        self.buffer_obs[1][idx] = obs[1]
        self.buffer_next_obs[0][idx] = next_obs[0]
        self.buffer_next_obs[1][idx] = next_obs[1]
    ...
    def sample(self, batch_size):
        ...
        samples = {
            "observations": (torch.from_numpy(self.buffer_obs[0][indices]).to(self.device), [self.buffer_obs[1][i] for i in indices]),
            ...
            "next_observations": (torch.from_numpy(self.buffer_next_obs[0][indices]).to(self.device), [self.buffer_obs[1][i] for i in indices]),
...
def optimize_policy(
    policy_fn: Callable[[np.ndarray, nn.Module, int, Optional[Any]], Tuple[int, Any]],
    optim_args: Args,
):
    ...
    q_policy = functools.partial(policy_fn, model=q_network)
    target_policy = functools.partial(policy_fn, model=target_network)
    ...
    for global_step in range(args.total_timesteps):
        ...
        # ASSUME NDIMS = 1 (ONLINE LEARNING )
        with torch.no_grad():
            #q_dist = q_network(torch.Tensor(obs).to(device))
            actions_and_mem, q_dist = zip(*[q_policy(o, memory=m) for o, m in zip(obs, memories)])
            (_, next_memories) = zip(*actions_and_mem)

            q_dist = torch.concat(q_dist, dim=0)
        ...
        real_next_memories = copy.deepcopy(next_memories)
        for idx, trunc in enumerate(truncations):
            if trunc:
                real_next_obs[idx] = infos["final_observation"][idx]
                real_next_memories[idx] = memories[idx]
        rb.add((obs, memories), actions, rewards, (real_next_obs, real_next_memories), terminations)
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    breakpoint()
                    _, next_dist = [target_policy(o, memory=m) for d, m in zip(data.next_observations)]  # [B, num_actions, n_atoms]
                    support = target_network.support  # [n_atoms]
                    next_q_values = torch.sum(next_dist * support, dim=2)  # [B, num_actions]

                    # double q-learning
                    next_dist_online = itertools.map(q_policy, data.next_observations)  # [B, num_actions, n_atoms]
                ...
                _, dist = itertools.map(q_policy, data.observations)  # [B, num_actions, n_atoms]
