...
#breakpoint()
strategy_recognition_prompt = "The video is no longer relevant, let's start developing a strategy to play the real game. Describe the ideal strategy if you were playing this game. What information might you want ot store between actions? Think about how you might structure an object to keep track of the state. Be specific. Use at most 100 words." #ATARI-GPT
...
code_prompt = "Given your plan, write the python code that plays the game to maximize the score. The code should take one" \
    + """ frame of the game provided as a grayscale numpy array shaped
    Box(0, 255, (210, 160), np.uint8) for each frame as input along with a weights array shapeed ((1, 10), np.float32) that you can use at your discretion to tune elements of your code and a memory array you can use to save information between successive calls to your code. Your code will
    return the action to take in the next time step and an updated memory array.""" \
    ...
    multiple frames to make your decisions so you may want to utilize the memory array to save information for later. The entry point for your code should be `def predict_next_action(frame: np.ndarray, weights: np.ndarray, memory: Optional[A CLASS OF YOUR DESIGN]) -> Tuple[int, CLASS OF YOUR DESIGN]:`""" \
    + "You are allowed to return any data you want in the second return of the function. This will be passed as input to the next call to your function in a closed-loop manner" \
    + """ For example this is how your code will be used:
    state = np.zeros(...)
    action = 0
    weights = np.randn((1,10))
    ob = env.reset() #Shape (210, 160)

    while True:
        action, state = predict_next_action(ob, weights, state)
        ob = env.step(action)
    """

code_prompt += "You are provided the following utility functions to help you implement your policy: " + str(UTILITY_SPECS)
code_prompt += "Only generate python code, do not generate any comments, do not provide explaination"

predict_next_action, _ = generate_and_load_policy(actor, action_space, code_prompt, exp_name="closed_loop")
...
def reset_env():
    ...
    env = gymnasium.wrappers.RecordVideo(
        ...
        video_folder="videos/closed_loop_policy_rollouts_" + ENV_NAME.split("/")[1],
...
for r in range(NUM_REFINEMENT_EPISODES):
    ...
    while not done:
        ...
        memory = None
        try:
            action_, memory = predict_next_action(obs, weights, memory)
            ...
            while True:
                predict_next_action, _ =  generate_and_load_policy(actor, action_space, regenerate_prompt, exp_name="closed_loop")
                try:
                    action_, memory = predict_next_action(obs, weights, memory)
                    _LOGGER.debug(action_)
                    action = int(action_)
        ...
        if len(action_history) == ACTION_HISTORY_LEN and all([action_history[0] == i for i in action_history]):
            ...
            predict_next_action, _ = regenerate_policy(actor, action_space, regenerate_strategy_prompt, [f"./videos/closed_loop_policy_rollouts_Breakout-v5/refine_{slugify(MODEL).replace('-', '_')}_policy-episode-0.mp4"], code_prompt, exp_name="closed_loop")
    ...
    predict_next_action, _ = regenerate_policy(actor, action_space, refine_prompt, [f"./videos/open_loop_policy_rollouts_Breakout-v5/refine_{slugify(MODEL).replace('-', '_')}_policy-episode-0.mp4"], code_prompt, exp_name="closed_loop")
    print(f"Refinement Episode {r} completed.")
    breakpoint()
    os.rename(f"./videos/closed_loop_policy_rollouts_Breakout-v5/refine_{slugify(MODEL).replace('-', '_')}_policy-episode-0.mp4", f"./policy_rollouts_Breakout-v5/refine_{slugify(MODEL).replace('-', '_')}_policy-episode-{r+10}.mp4")
exit()
